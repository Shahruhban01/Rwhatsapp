...
import { X, User, Shield, Laptop, Palette, LogOut, Check, Edit2, Loader2, Key } from 'lucide-react';
...
type Tab = 'profile' | 'linked-devices' | 'privacy' | 'theme';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  // Load active sessions when tab changes to linked-devices
  useEffect(() => {
    if (activeTab === 'linked-devices' && isOpen) {
      fetchSessions();
    }
  }, [activeTab, isOpen]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-[800px] h-[550px] bg-[#111b21] border border-[#222e35] rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row text-[#efeae2]">
        ...
        {/* Left Tabs Sidebar */}
        <div className="w-full md:w-[240px] bg-[#202c33]/40 border-r border-[#222e35] flex flex-col p-4">
        ...
        <div className="flex-1 flex flex-col p-6 min-w-0 bg-[#111b21] relative">
          ...
          <div className="flex-1 overflow-y-auto pr-2 mt-4 md:mt-0">
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6">
                ...
                {/* Avatar Display */}
                <div className="flex items-center gap-4 bg-[#202c33]/30 p-4 rounded-xl border border-[#202c33]/50">
                  <div className="w-16 h-16 rounded-full bg-[#00a884] flex items-center justify-center font-bold text-white uppercase text-2xl select-none">
                    {user?.name?.[0] || 'U'}
                  </div>
                  <div>
                ...
                <div className="space-y-2">
                  <label className="text-xs text-[#8696a0] font-semibold uppercase tracking-wider">Display Name</label>
                ...
                {/* About Status Field */}
                <div className="space-y-2">
                  <label className="text-xs text-[#8696a0] font-semibold uppercase tracking-wider">About Status</label>
                  <div className="flex items-center gap-3">
                    {isEditingAbout ? (
                      <div className="flex items-center gap-2 w-full">
                        <input
            ...
            {activeTab === 'privacy' && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold text-[#f0f2f5] mb-1">Privacy Controls</h3>
                  <p className="text-xs text-[#8696a0]">Control who can see your activity details and read receipts.</p>
                </div>

                <div className="space-y-4">
                  {/* Last Seen toggle */}
                  <div className="flex justify-between items-center bg-[#202c33]/30 p-4 rounded-xl border border-[#202c33]/50">
                    <div>
                      <h4 className="text-sm font-semibold text-[#e9edef]">Share Last Seen Status</h4>
                      <p className="text-[11px] text-[#8696a0] mt-0.5">Allow other users to see when you were last online</p>
                    </div>
                    <button
                      onClick={() => setLastSeenPrivacy(!lastSeenPrivacy)}
                      className={`w-11 h-6 rounded-full transition-colors relative flex items-center px-1 ${lastSeenSeenClass(lastSeenPrivacy)}`}
                    >
                      <span className={`w-4 h-4 rounded-full bg-white shadow-md transition-transform ${lastSeenPrivacy ? 'translate-x-5' : 'translate-x-0'}`} />
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center bg-[#202c33]/30 p-4 rounded-xl border border-[#202c33]/50">
                    <div>
                      <h4 className="text-sm font-semibold text-[#e9edef]">Send Read Receipts</h4>
                      <p className="text-[11px] text-[#8696a0] mt-0.5">Toggle blue ticks for sent and received messages</p>
                    </div>
                    <button
                      onClick={() => setReadReceipts(!readReceipts)}
                      className={`w-11 h-6 rounded-full transition-colors relative flex items-center px-1 ${lastSeenSeenClass(readReceipts)}`}
                    >
                      <span className={`w-4 h-4 rounded-full bg-white shadow-md transition-transform ${readReceipts ? 'translate-x-5' : 'translate-x-0'}`} />
                    </button>
                  </div>
                </div>
              </div>
...
// Helper style
const lastSeenSeenClass = (val: boolean) => val ? 'bg-[#00a884]' : 'bg-slate-600';
